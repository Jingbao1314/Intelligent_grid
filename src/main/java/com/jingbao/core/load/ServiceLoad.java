...
import com.jingbao.base.MyException.UrlRepeatException;
...
public class ServiceLoad {
        ...
        if(clazz.isAnnotationPresent(Service.class)){
            for (Method m :clazz.getMethods()) {
                if (m.isAnnotationPresent(RequestMapping.class)){
                    ...
                    ServiceEntity serviceEntity=new ServiceEntity(m,clazz,m.getParameterTypes());
                    if (urlMapping.get(mapping.value())==null){
                        urlMapping.put(mapping.value(),serviceEntity);
                    }else {
                        throw new UrlRepeatException("重复的URL MAPPING");
                    }
                }
            }
        }
    ...
    public static void main(String[] args) {
        List<Class<?>> classList = ServiceLoad.getClasssFromPackage("com.jingbao.core.load");
